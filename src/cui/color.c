...
/*   Updated: 2021/01/24 16:29:18 by archid-          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	assert(at < MEM_SIZE);
	return ((g_memcolors[at] >> 4) ? ((g_memcolors[at] >> 4) + color_count) : 0);
}

inline void			set_color(t_u8 clr, t_u16 at)
{
	assert(at < MEM_SIZE);
	g_memcolors[at] = (g_memcolors[at] & 0xf0) | (clr & 0x0f);
}

inline void			set_color_effect(t_u8 clr, t_u16 at)
{
	assert(at < MEM_SIZE);
	g_memcolors[at] = (g_memcolors[at] & 0x0f) | ((clr & 0x0f) << 4);
}
