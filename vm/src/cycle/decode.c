...
/*   Updated: 2021/03/15 09:40:35 by archid-          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_st		vm_decode(t_proc p, t_pc *offset)
{
	if (p->op.info.meta.of.encoded)
	{
		...
		if (g_show_logs)
			ft_dprintf(g_fd, " >>>	 encoding of %s is %08b %08x\n",
						p->op.info.name, p->op.info.encoded.encod,
						p->op.info.encoded.encod);
		*offset += 2;
	...
	{
		*offset += 1;
		mem_read_chunk(p, &p->op.info.args.c[0], p->op.callback == live,
						*offset);
